...
import { User, NFT, sequelize, InstagramUser } from '../database/models';
...
export const userController = () => {
  const signUp = async (req, res, next) => {
    try {
      ...
      let { wallet_addr, firstName, lastName, email, phoneNumber, publicName, password } = req.body;
  ...
  const getProfileByUserId = async (req, res, next) => {
    try {
      ...
      const user = await User.findOne({
        ...
        attributes: { exclude: ['encryptedPassword'] },
        include: [{ model: InstagramUser, as: 'instagramUser' }],
      });
      ...
      if (user) {
        ...
        if (instagramUser) {
          ...
          const error = new Error(errorMessages.NO_USERNAME);
          error.status = errorCodes.BAD_REQUEST;
          throw error;
        }
      }
  ...
  const validate = (method) => {
    switch (method) {
      case 'signup': {
        return [
          body("firstName", errorMessages.FIRST_NAME_REQUIRED).exists(),
          body("lastName", errorMessages.LAST_NAME_REQUIRED).exists(),
          body('email').exists().withMessage(errorMessages.EMAIL_REQUIRED).isEmail(),
        ];
      }
      case 'get-instagram-username': {
        return [body('accessToken', errorMessages.ACCESSTOKEN_REQUIRED).exists()];
      }
      case 'login': {
        return [
          body('email')
